...
public class DBThread extends Thread 
{
	...
	private static final Logger logExec = Logger.getLogger("SQLEXEC");
	...
	public DBThread() {
		...
		dbEngine = new DBEngine("conn_ppsea", true);
	}
	...
	public void logsql(OperaInfo oper) 
	{
		try
		{
			log.debug("|" + GameServer.zoneId + "|" 
					+ oper.getSequenceNo() + "|Q:"
					+ operaQueue.size() + "|M:" 
					+ cachedData.size() + "|" 
					+ oper.typeToString() + "|" 
					+ oper.getTable() + "|" 
	...
	public String toSql(OperaInfo op, boolean insert) 
	{
		...
		if (op.getEop() == OperaInfo.EOpera.UPDATE
			|| op.getEop() == OperaInfo.EOpera.NO_CACHE_UPDATE)
		{
			String opera = "update ";
			if (insert) opera = "insert into ";
			...
			if (!insert) sql.append(" where " + op.getWhere());
		}
		else if (op.getEop() == OperaInfo.EOpera.NO_CACHE_UPDATE_WHERE)
		{
			...
			sql.append(" where " + op.getWhere());
				 ...
				 || op.getEop() == OperaInfo.EOpera.NO_CACHE_DELETE) 
		{
			sql.append("delete from " + op.getTable() + " where id=" + op.getId());
		} 
		else if (op.getEop() == OperaInfo.EOpera.DELETE_WHERE
				||op.getEop() == OperaInfo.EOpera.NO_CACHE_DELETE_WHERE)
		{
	...
	public int exec(OperaInfo op, boolean isNoCache) 
	{
		...
		try
		{
			...
			//--------------------------------------------------
			logExec.debug("|" + GameServer.zoneId + "|"
					+ op.getSequenceNo() + "|"
					+ operaQueue.size() + "|" 
					+ cachedData.size() + "|" 
					+ op.typeToString() + "|" 
			...
			//--------------------------------------------------
			if (rows == 0 && 
				(op.getEop() == OperaInfo.EOpera.UPDATE
				 || op.getEop() == OperaInfo.EOpera.NO_CACHE_UPDATE))
			{
				sql = toSql(op, true);
				rows = dbEngine.executeUpdate(sql);
	...
	public void run() 
	{
		...
		while (!terminated)
		{
			try
			{
				...
				if (lastSyncDbTime + 5000 < System.currentTimeMillis()) {
					...
					logExec.debug("curr qsize:" + operaQueue.size() + 
						     ",curr msize:" + cachedData.size() + 
						     ",proc succ:" + allKeys.size() + 
						     ",proc fail:" + errKeys.size() + 
						     ",lastSyncTime=(" + now.toString() + " - " + delayTime + "s)");
				}
			}
			catch (Exception e)
			{
		...
		logExec.info("DbThread sync2db done,m:" + cachedData.size() 
				+ ",q:" + operaQueue.size() + ",delay:" + delayTime);
		////////////////////////////////////////////////////////////
	}
	...
	public static class OperaInfo 
	{
		enum EOpera { 
			...
			NO_CACHE_UPDATE,
			NO_CACHE_DELETE,
			NO_CACHE_UPDATE_WHERE,
			NO_CACHE_DELETE_WHERE,
		}
		...
		///////////////////////////////////////////////////////////////////
		public static String getKey(String tab, int iden) { return tab + "-" + iden; }
		public String getKey() { return table + "-" + id; }
		...
		public String typeToString(){ 
			if (this.eop == EOpera.UPDATE) return "UPDATE";
			if (this.eop == EOpera.DELETE) return "DELETE";
			//if (this.eop == EOpera.UPDATE_WHERE) return "UPDATE_WHERE";
			if (this.eop == EOpera.DELETE_WHERE) return "DELETE_WHERE";
			if (this.eop == EOpera.NO_CACHE_UPDATE) return "NO_CACHE_UPDATE";
			if (this.eop == EOpera.NO_CACHE_DELETE) return "NO_CACHE_DELETE";
			if (this.eop == EOpera.NO_CACHE_UPDATE_WHERE) return "NO_CACHE_UPDATE_WHERE";
			if (this.eop == EOpera.NO_CACHE_DELETE_WHERE) return "NO_CACHE_DELETE_WHERE";
			return "";
		}
		///////////////////////////////////////////////////////////////////		
		...
		public EOpera getEop() { return eop; }
		public void setEop(EOpera eop) { this.eop = eop; }
