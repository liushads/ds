...
import com.ppsea.ds.service.DBThread.OperaInfo;
...
public class DBService {
	 ...
	 * @return
	 */
	public static String getResultSetName(String clazz) {
		return StringUtil.unix_simple_clazz(clazz) + ".ibatorgenerated_BaseResultMap";
	}
	public static String getResultSetNameBlob(String clazz) {
		return StringUtil.unix_simple_clazz(clazz) + ".ibatorgenerated_ResultMapWithBLOBs";
	}
	/**
	 * 
	 * @return
	...
	public static void commitDelete(BaseObject obj) 
	{
		try
		{
			...
			opera.setTable(StringUtil.unix_simple_clazz(clazz));
	 ...
	 * @param keyId
	 */
	public static String getNotNullFields(BaseObject obj,
...

	{
		...
		*/
		if (!orz.containsKey(clazz)) {
			...
			ResultMap rmBlob = null;
			try{
				rmBlob = delegate.getResultMap(DBService.getResultSetNameBlob(clazz));
			}catch(com.ibatis.sqlmap.client.SqlMapException e){
			}
			
			if(rmBlob == null){
				ResultMap rm = delegate.getResultMap(DBService.getResultSetName(clazz));
				orz.put(clazz, rm.getResultMappings());
			}else{
				orz.put(clazz, rmBlob.getResultMappings());
			}
		}
		...
		try 
		{
			for (int i = 0; i < rmps.length; ++i) 
			{
				...
				else if (type.equals("BIT"))
				{
					...
					if (val.equalsIgnoreCase("false")) val = "0";
				}
				if (val != null) {
	...
	public static boolean flush(String className, int id) {
		try
		{
			...
			*/
			String key = OperaInfo.getKey(StringUtil.unix_simple_clazz(className), id);
	...
	public static DBThread.OperaInfo getUpdateOperaInfo(BaseObject obj) {
		...
		try
		{
			...
			String tab = StringUtil.unix_simple_clazz(clazz);
			if (tab == null || tab.equals("null")) {
				log.error("getUpdateOperaInfo tab error:" + clazz);
			}
			String whr = " id='" + id + "'";
			DBThread.OperaInfo opera = new DBThread.OperaInfo();
			if(id != null)opera.setId(Integer.valueOf(id));
	...
	public static void main(String[] argv) {
		...
		Test t = new Test();
		byte[] bs = {14, 11, 12, 13, 127, -126};
		t.setId(100);
		t.setContent(bs);
		DBService.commitNoCacheUpdate(t);
	}
	*/
}
